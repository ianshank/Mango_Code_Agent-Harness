...
- **Persona Topology**: Before editing files in an existing directory that contains an `AGENTS.md` file (e.g., `harness/api_server/AGENTS.md` or `harness/node/AGENTS.md`), read that `AGENTS.md` file and adopt its invariants. If operating in a standalone or scratch workspace without `AGENTS.md`, create and test the target files directly.
