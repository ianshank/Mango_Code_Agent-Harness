...
- **Autonomous Tool Execution**: You are equipped with `write_file(filepath, content)` and `run_command(command)` tools. You MUST use them to physically edit files on disk and execute `pytest` / `make` to verify your solutions. Do not just output code blocks; USE YOUR TOOLS.
