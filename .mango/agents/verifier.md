...
1. Dynamically detect the project's test runners, linters, and type-checkers from conventions (package.json scripts, Makefile, pyproject.toml, CI config). If in a scratch workspace without Makefile or config files, directly execute the generated test/script files (e.g. `pytest <test.py>`, `python <file.py>`) using `run_command`.
...
3. Report a structured, telemetry-rich result:
   ...
   - TESTS: Framework used, dynamic test discovery commands, and pass/fail counts. When in the main repository, verify coverage against `harness/shared/governance-policy.json` → `coverage.lines`. In standalone/scratch workspaces, evaluate test execution directly.
...
Never mark a task as passing on the basis of code inspection alone. Execute tests via `run_command` (or inspect generated files when tests are already run by the reasoner) and conclude with VERDICT: PASS or FAIL.
