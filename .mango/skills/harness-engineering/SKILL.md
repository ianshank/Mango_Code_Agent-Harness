...
## When invoked
1. Treat this repository's `.claude/` directory as the source of truth for current
...
4. If a failure pattern recurs (e.g. the agent repeatedly ignores a constraint), record it
   in `.claude/FAILURE_MEMORY.md` with: what happened, why, and what check now prevents it.
