...
lint-python: ## Run ruff check + mypy across all first-party Python (sources, tools, and tests)
	$(RUFF) check .
	$(MYPY) $(SHARED_SRC) harness/api_server --explicit-package-bases $(MYPY_FLAGS)

.PHONY: lint-cold
lint-cold: ## Typecheck with no mypy cache — CI always runs cold, the inner loop does not
	$(MYPY) $(SHARED_SRC) harness/api_server --explicit-package-bases $(MYPY_FLAGS) --no-incremental
