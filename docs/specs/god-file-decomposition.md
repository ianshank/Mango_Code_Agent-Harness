...
- [x] AC-2: `mango_mas_orchestrator.py` decomposed (prompts, tool dispatch, tool execution extracted to R-GFD-1/2/3 modules) and covered by the governance-policy `size_budget_lines` gate; the file has since grown to 483 lines (re-measured 2026-08-31; was 465 at an earlier check) under organic feature work (read_file/apply_patch, docs/specs/agent-read-patch-tools.md) and is now within 17 lines of the enforced 500-line budget. The original "<300 lines" figure predates that budget and is superseded by it — tracked, not reopened, since R-ORCH-1..4 (`docs/specs/orchestrator-tool-registry.md`) already cover further internal decomposition of `execute_agent`. That spec's acceptance criteria are still all unchecked (not yet implemented); given the shrinking headroom, prioritizing it is now closer to necessary than optional.
