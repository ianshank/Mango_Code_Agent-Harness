...
| 4 | Structure without reopening DEC-020: test size budget, `nemotron-client.ts` split, docs consolidation, bootstrap dedup or DEC | 4 | medium (protected paths) | **yes**: whether DEC-020 is ever reopened |
...
| Regroup `harness/shared/` into four packages | Reverses DEC-020 without citing it; the split is cyclic (`tooling ↔ runtime`, `tooling → gates`); eight modules unassigned | decision log line 25; `tool_executors.py:239` → `debug_dump`, `agent_prompts.py:5` → `agent_authority`, `write_policy.py:37` → `validate_invariants` | Dropped. DEC-020 stands (R-TDH-20). Reopening needs a superseding DEC and an acyclicity test first |
| Delete the 20 per-stack shim scripts | They are root-of-trust artefacts: listed in `harness/node/.governance/policy.json:86-96`, digested in `policy-bundle.example.json:20-31`; DEC-004 sizes removal as a rotation; two `per-file-ignores` would go dead | as cited; `pyproject.toml:110-111`; `test_lint_config_liveness.py:92-102` | Dropped (R-TDH-21 keeps them) |
| Six required checks | Nine; already derived from the workflow by `test_ci_gate_coverage.py::TestRequiredStatusChecksListIsAccurate` | `NEXT_STEPS.md:258-261`; DEC-018 | R-TDH-1 cites the test, restates nothing |
...
| `AgentLoop` defaults from `agent_defaults()` | Class is `ExecutionLoop`; accessor is `policy_loader.max_tool_calls_per_task()`; a dataclass-style default would read the filesystem at import | `orchestrator/loop.py:32`; `policy_loader.py:138,169-182`; `test_import_purity.py` | R-TDH-12: `None` defaults resolved at call time |
| `GraphPolicy()` reads policy at construction | Contradicts landed `langgraph-policy-wiring` R-LPW-4/5 (pure fallback; `from_governance_json()` is the policy path) | `docs/specs/langgraph-policy-wiring.md:83-93`; `graph.py:69`, `nodes.py:225` | R-TDH-12 pins the eleven defaults equal to `policy_loader` fallbacks by an equality test |
| `"coverage": 85.0` is a fabricated gate input | It is inert: `quality_gate_node` reads only `failed` | `nodes.py:195,244-247`; `test_langgraph_nodes.py:168-190` | R-TDH-15 removes the stub; wiring coverage floors into the gate is a separate behavioural spec (open question 5) |
| `decision_id_pattern` duplicated fallback regex | Both scripts already read the policy first and fail closed; the literal is the adopter path `policy-single-source.md` R-POL-4 requires, lockstep-pinned by `test_policy_consistency.py:22,260` | `check_projections.py:66-71`; `verify_zero_skips.py:70-75` | Dropped: checked, not a finding |
| Four "verified-dead" symbols | Zero first-party callers is correct, but two are declared compat exports and `resolve_api_key` gates three live-detection test modules | `write_policy.py:95-97`; `nemotron_bridge.py:78-81`; `test_mango_mas_live.py:9` | R-TDH-17 treats them as a deprecation, not deletion |
| Facade keeps only constructor and `execute_agent` | `autonomous_healing.py:121` calls `execute_loop`; `nodes.py:185` calls `_harness_verdict` | as cited | Backward compatibility section corrected |
...
| Ruff bump justified by "rule counts change" | Counts are lower-bounded, not asserted; the real reason is 7 new findings, two in protected `write_policy.py` | `python -m ruff check .` under 0.16.5; `test_deferred_rigor.py:192-200` | R-TDH-10 rationale corrected |
| Python zero-skip gate needs `pytest-json-report` | `verify_zero_skips.py` already accepts `--junit-events` TSV | `verify_zero_skips.py:143-178` | R-TDH-19 reuses it; a `conftest.py` hook emits the TSV |
...
| Missed entirely | All five Actions run on Node 20; no `github-actions` Dependabot ecosystem; nightly drift never runs `lint`, so the mypy break was invisible nightly; `api_server/main.py:13-15` imports root modules; `test_orchestrator_hooks` POSIX skips | workflow `:109`; `dependabot.yml`; `scheduled-drift.yml:68`; `pytest -rs` | R-TDH-10, R-TDH-11, R-TDH-19 |

Verified correct and kept: the five local failures and their causes; the three
live policy divergences (`loop.py:45-47`, `nemotron-client.ts:25`, five version
strings); the 32 langgraph regression cases skipped in CI; `remotes.py` at 46
lines (but *not* over any budget that applies to it); nine watch-list modules,
none over budget; 920-line `test_ci_gate_coverage.py`; 24.7 k test lines vs
...
(`scheduled-drift.yml:51-53`, DEC-018, `NEXT_STEPS.md:253`: "the highest-value
...
|---|---|---|
| `build (3.10)`, `build (3.12)` | mypy | `mcp_server.py:120` passes `inputSchema=`; `9d38670` reverted the `input_schema` rename that `2ffd228` landed under DEC-023 |
...
`orchestrator/loop.py` in `9d38670`); two `test_documentation_truth` failures
...
`orchestrator/loop.py:45-47` defaults `15/30/50` vs policy `10/300/100`
(masked: the facade passes explicit values); `nemotron-client.ts:25`
...
caller; `autonomous_healing.py` and `lats_optimizer.py` (v2.3.0 headline
...
nine are past 60 % (`plan_rules.py` 428 the largest). Tests are outside the
budget; `test_ci_gate_coverage.py` is 920 lines and the largest Python file
in the repository.

**7. Toolchain.** Every GitHub Action in the workflow runs on Node 20 (deprecated
on the runners); `dependabot.yml` covers `pip` and `npm` only; nightly drift
...
- R-TDH-1: `main` MUST carry a repository ruleset requiring the nine status
  checks `test_ci_gate_coverage.py::TestRequiredStatusChecksListIsAccurate`
...
- R-TDH-3: `harness/shared/mcp_server.py` and `test_mcp_server.py` MUST use
...
- R-TDH-7: `pyproject.toml` `[project].version` MUST be the single version
  ...
  `NEXT_STEPS.md` are compared to it by `test_documentation_truth.py`, which
...
- R-TDH-9: CI MUST install Python dependencies from a committed
  ...
  and a new unprotected `test_workflow_contracts.py` failing when an install
...
- R-TDH-10: Dependabot PRs #38–#46 MUST each be merged or closed after Phase 0b,
  ...
  tree, two in protected `write_policy.py`; the `measured` values in
  `test_deferred_rigor.py` are refreshed in the same PR); `dependabot.yml`
...
- R-TDH-12: `ExecutionLoop.__init__` in `harness/shared/orchestrator/loop.py`
  ...
  equality test in the `test_policy_consistency.py` pattern (the pure
...
- R-TDH-13 (*child spec* `node-policy-wiring`):
  `harness/node/src/ai/nemotron/nemotron-client.ts` and `cli.ts` MUST read
  ...
  `vitest.config.ts` does, throwing on a missing key, with a Vitest liveness
  test.
- R-TDH-14: Verdict status strings in `governance/broker.py`,
  ...
  reference `governance/verdict.py` constants; an AST-based test fails on a
  raw `"BLOCKED"`/`"FAILED"`/`"VERIFIED"` string literal outside that module,
  exempting `Literal[...]` annotations.
- R-TDH-15: `langgraph/nodes.py` MUST NOT emit the stub `"coverage": 85.0`;
  ...
  change to protected `langgraph/**` and is open question 5, not this plan.
...
- R-TDH-17: `write_policy.ALWAYS_DENIED_PREFIXES`,
  ...
  `langgraph/nodes.py:23-25` removed, and `vulture` pinned in
...
- R-TDH-18: `autonomous_healing.py` and `lats_optimizer.py` MUST be either
...
- R-TDH-19: Python skips MUST be accounted for (INV-2 parity): the four
  empty-parametrize skips (`test_ci_gate_coverage.py:526`,
  `test_import_purity.py:174,185`, `test_test_quality.py:120`) are fixed; a
  `conftest.py` `pytest_runtest_logreport` hook writes
  ...
  skip, the POSIX-only skips in `test_orchestrator_hooks.py`/`conftest.py`
...
- R-TDH-22: Test modules MUST be subject to `limits.test_size_budget_lines`
  ...
  `test_ci_gate_coverage.py` is split by concern first, its exact
  `protected_paths` entry becomes a glob, and `test_protected_path_liveness.py`
  is updated in the same PR.
- R-TDH-23: The watch-list modules MUST stay under `limits.size_budget_lines`,
  and `nemotron-client.ts` (454 lines) MUST have retry/backoff extracted with
...
- R-TDH-24: Documentation MUST be consolidated: the v2.2.4 body moves from
  ...
  two C4 documents each merge; `test_documentation_truth.py` caps any
  `## [x.y.z]` section at `limits.changelog_section_lines`. `NEXT_STEPS.md`
  stays where `test_ci_gate_coverage.py` reads it.
...
- R-TDH-25: Every Phase 2/3 slice MUST bring branch tests for the missing arcs
  ...
  `check_dedup.py` 92.7 %, `mango_mas_orchestrator.py` 93.8 %,
  `orchestrator/hook_runner.py` 93.9 %, `check_py_compat.py` 94.4 %,
  `coverage_gate.py` 94.9 %, `governance/verify_zero_skips.py` 95.9 %; with
...
- [ ] AC-1: the ruleset export committed as `.github/rulesets/main.json` lists
      ...
      derives, asserted by a new test in `test_workflow_contracts.py` that
...
- [ ] AC-4: `pytest harness/shared/tests -m "not live and not langgraph" --co -q`
      ...
      (asserted by a test in `test_test_quality.py`); with `langgraph`
      installed `pytest -k test_state_graph_healing_loop_recovers_from_test_failure`
      passes rather than skipping; `test_workflow_contracts.py` fails if the
...
- [ ] AC-11: `pytest harness/shared/tests/test_workflow_contracts.py -k drift`
      fails if `scheduled-drift.yml`'s `main-drift` loop omits `lint`
...
- [ ] AC-14: `pytest harness/shared/tests -k verdict_literals` fails on a raw
      verdict string literal outside `governance/verdict.py` (negative case via
...
- [ ] AC-26: `pytest harness/control-plane/tests` collects at least one test
      ...
      (`test_makefile_contracts.py`) · stage: `make test-python` (R-TDH-26)
...
1. Apply the ruleset from the nine names (admin); commit its export; add the
   ...
   `GOVERNANCE_SKILL.md`; add the "claims are not evidence" rule to
...
4. `uv pip compile --universal` lock; workflow install and cache keys;
   `test_workflow_contracts.py` (R-TDH-9). Protected: workflows.
...
7. `ExecutionLoop` call-time defaults; `GraphPolicy` equality pin
   (R-TDH-12). Protected: `langgraph/**`, `policy_loader.py`.
...
9. Verdict constants and AST test; remove the coverage stub; constant triage
   ...
   Protected: `governance/**`, `governance-policy.json`.

### Phase 3 (3 PRs)

10. Deprecation shims, `RunnableConfig` removal, `vulture` in `make lint`
    (R-TDH-17). Protected: `write_policy.py`, `Makefile`,
    `requirements-dev.txt`.
11. LATS/healing per open question 3; retarget facade tests; delete
    pass-throughs (R-TDH-18). Protected: `mango_mas_orchestrator.py`.
...
13. Record R-TDH-20 in the decision log; bootstrap helper or accepting `DEC-`
    (R-TDH-20, R-TDH-21).
14. Split `test_ci_gate_coverage.py`; `check_test_size_budget` and key; glob
    the protected entry (R-TDH-22). Protected: `validate_invariants.py`,
    `governance-policy.json`, `test_ci_gate_coverage.py`.
15. `nemotron-client.ts` split and ESLint `max-lines` (R-TDH-23).
...
`execute_loop` (used by `autonomous_healing.py:121`) and `_harness_verdict`
(used by `langgraph/nodes.py:185`), and only the four test-only pass-throughs
