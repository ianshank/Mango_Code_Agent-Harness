...
| M4 | Meta-tools are write-only and install-path-scoped: nothing reads `knowledge_gap_log`/`hypothesis_register` output; `MEMORY_DIR` is resolved from the harness install path so all workspaces share one store. `[Certain]` | `meta_tools.py:20-22,117-166` | Feed open gaps into the next planner prompt; scope the store under the workspace. **Remediated in part.** NS-17 scoped the store per workspace and surfaced open gaps to the planner (`format_gaps_for_planner`). DEC-057 made the hypothesis store self-reading — a revision supersedes the entry it names — so "write-only" no longer holds of the file. **Fully remediated by DEC-058:** the reasoner prompt now carries the workspace's open hypotheses through `memory_view.format_hypotheses_for_reasoner`, bounded by `agent_memory.reasoner_hypothesis_limit` / `reasoner_hypothesis_budget_tokens` and proven to coexist with H4 eviction (`docs/specs/hypothesis-surfacing.md`); `C-HR-2` narrowed to `C-HS-1`. |
