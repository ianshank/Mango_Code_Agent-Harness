# Milestone history — v2.1.3 through v2.4.0

**Archived from `NEXT_STEPS.md`, 2026-09-03.** This is the completed-milestone
record that used to occupy 496 of that file's 533 lines, moved here verbatim so
the roadmap can be read as a roadmap. Nothing is edited: the `🚧` sections and
their unchecked boxes are preserved exactly as they stood, including the claims
later corrected elsewhere.

**This file is a snapshot, not a tracker.** Every item that was still open when
this was archived was carried forward into `NEXT_STEPS.md` with its evidence
re-verified against the tree; an unchecked box *here* is history, and the
authoritative status of that work is whatever `NEXT_STEPS.md` says today. The
narrative record of what shipped lives in `CHANGELOG.md` and `docs/releases/`;
this file exists to preserve the roadmap's own prior wording, which in several
places is the only surviving statement of why a decision was taken.
...
- [x] **Deterministic Sandbox E2E Matrix (`test_neurosym_sandbox_e2e.py`)**: Verified `INV-9` fail-closed backend checks and `AC-NS-3` multi-turn critique repair loops. The `AC-CE-1` capability-profile violation in these tests is *simulated by a mock backend*; the production `ProcessBackend` does not enforce capability profiles (`docs/architecture/c4_architecture.md` §4.6), so `AC-CE-1` itself remains open in the code-execution spec.
