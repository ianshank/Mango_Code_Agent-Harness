...
- **NS-36 minus signatures:** attestation binds to PR head SHA; Dockerfile digest-pins `node:26-alpine`, runs as `USER node`, drops `EXPOSE`/`tsx`; Dependabot docker `cooldown`; scheduled `protection_report` queries `/rules/branches/main` and stays green on query/API hiccups (`set +e`, empty-list only).
