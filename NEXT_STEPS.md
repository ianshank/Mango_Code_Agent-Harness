...
plan (`docs/specs/2026-standards-remediation-plan.md`, R-SR-1 GÇª R-SR-31) carries
...
|---|---|---|---|
| PR-1 | **Blocker (doc)** | NS-32 said "Land Phase B (PR #86)" while the open roadmap still treated Phase B as unfinished. PR #86's title is `docs(reports): 2026 coding-standards audit`, but its body and 97-file diff landed Phase B (R-SR-6GÇªR-SR-22 / AC-6GÇªAC-22, AC-33 `[x]`); DEC-048GÇªDEC-051 carry the containment/runtime narrative. | NS-32 closed GåÆ -º6 Delivered; remediation plan bumped to rev 2 (Phase B Done). |
...
| PR-4 | Unchanged blockers | Re-queried 2026-09-05: `GET GÇª/rules/branches/main` GåÆ `[]`; `feature/governed-run-console` still at `5970249GÇª`; zero tags; `license: null`. | NS-1, NS-2, NS-3, NS-30 stay P0. |

### 2026-09-05b second peer-rewrite findings (verified)

| ID | Severity | Finding | Effect on this file |
|---|---|---|---|
| PR-5 | **Blocker (doc)** | NEXT_STEPS still listed NS-11 / NS-31 / NS-33 as open; header cited `2441547` not tip `58490c1`. | Move NS-11 / NS-31 / NS-33 to -º6; header retargeted to tip. |
| PR-6 | **Blocker (doc)** | Remediation plan still revision 2 @ `2441547`; AC-5 unchecked; Phase E prose conflated R-SR-5 Done with R-SR-2 still gating. | Plan GåÆ revision 3; AC-5 `[x]`; Phase E gated on NS-2 / R-SR-2 only. |
...
| PR-8 | Major | NS-4 still said bot PRs left for maintainer; Dependabot open = none (closed #62-#78). | -º6 NS-4 disposition updated. |
| PR-9 | Major | -º4 Phase E still blocked on NS-31; DEC-053GÇª056 exist on #93. | Phase E retargeted to **NS-2** before destructive slices. |
...
| PR-11 | Product | NS-17 open on #97; Copilot: `policy_path` not plumbed into `agent_memory_defaults` / gap injection. | **Closed:** #97 landed `policy_path` + mutation + zero-bound messaging; moved to -º6 with NS-34. |
| PR-12 | Unchanged P0 | Re-queried: ruleset `[]`; tip `58490c1`; branch `5970249GÇª`; 0 tags; `license: null`. | NS-1 / NS-2 / NS-3 / NS-30 stay P0. |
...
**Done when.** Settings GåÆ Rules GåÆ Rulesets GåÆ Import
`.github/rulesets/main.json` (with `required_signatures` and
`required_linear_history` added first, R-SR-1), and
`GET GÇª/rules/branches/main` returns a non-empty list; a PR whose head has a
...
leaked key". The branch is still on the remote at `5970249GÇª` (re-queried
...
destructive slice.** DEC-053GÇª056 are logged; they do not lift this gate.
...
**Evidence.** API `license: null`; `ls LICENSE*` GåÆ none.
...
on PR #93** (DEC-053 PARK GÇª DEC-056); do not revive an "undecided" NS-31 item.
...
(`Bash`, `Read`, GÇª) and is fed verbatim to Nemotron; only `run_command` matches
...
| Item | Blocked on |
|---|---|
| **Phase E** (R-SR-26 GÇª R-SR-29) | **NS-2 / R-SR-2 before any destructive slice.** DEC-053GÇª056 (NS-31 / R-SR-5 / AC-5) are logged on #93 - PARK order stands: **JVM GåÆ LangGraph GåÆ openspec GåÆ mirroring**. Do not start Phase E code while the DEC-014 credential branch remains. Premature Phase E inverts DEC-024 (claimed readiness without the hard gate). |
...
| **HITL interrupts** | Needs an explicit non-graph design under DEC-053 PARK (in-graph interrupts stay with a revival DEC). Context-window budget (audit H4) is no longer parked GÇö see -º6 / PR #110. |
...
| Was | Now |
|---|---|
| **NS-34** Decision records as records | **Landed on PR (this change).** One file per decision under `docs/decisions/` with YAML frontmatter (`status`, `supersedes`, GÇª); generated `index.md` / `index.json`; `validate_governance_docs` fails on missing status, index drift, or skill restatement; skill points at the index; thin legacy `decision-log.md` retains DEC ids for `--decision-log` consumers. |
...
| Was | Now |
|---|---|
| **NS-32** Land Phase B (PR #86) | **Landed on PR #86.** Title was docs-only (`docs(reports): 2026 coding-standards audit`), but the PR body and 97-file diff delivered Phase B (R-SR-6GÇªR-SR-22; AC-6GÇªAC-22, AC-33 `[x]`). Runtime/containment narrative: DEC-048GÇªDEC-051. Remediation plan rev 2 marked Phase B Done. Do not re-open under a different PR label. |
...
NS-22GÇªNS-28, gate half of NS-3, bound half of NS-9 - see git history of this
file at `6f0f18b`GÇª`58490c1` and `docs/reports/ROADMAP-PEER-REVIEW.md`.

**Corrected record (do not re-open as "unwired"):**

- `@with_authority` / `@budgeted` are applied **and**, after DEC-052 / PR #87-#88,
  denials fail closed through the compiled graph (INV-LG-6). Applied Gëá enforcing
