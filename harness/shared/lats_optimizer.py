...
stays side-effect free (``test_import_purity.py``); ``from ... import LATSOptimizer``
resolves through ``__getattr__`` (PEP 562) and warns once per access.
Import ``harness.shared.experimental.lats_optimizer`` instead.
"""
...
_MOVED_NAMES = ("LATSOptimizer",)

_MESSAGE = "harness.shared.lats_optimizer moved to harness.shared.experimental.lats_optimizer (DEC-027)"


def __getattr__(name: str) -> object:
    if name in _MOVED_NAMES:
        from harness.shared.experimental import lats_optimizer as _target
