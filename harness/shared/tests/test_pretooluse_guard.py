...
def test_destinations_explicit_remote(tmp_git_repo: Path):
    subprocess.run(["git", "-C", str(tmp_git_repo), "remote", "add", "origin", "https://github.com/org/repo.git"], check=True)
...
def test_destinations_skips_git_options(tmp_git_repo: Path):
    subprocess.run(["git", "-C", str(tmp_git_repo), "remote", "add", "origin", "https://github.com/org/repo.git"], check=True)
    seg = ["git", "-C", "/some/path", "--git-dir", ".git", "--work-tree", ".", "-c", "http.sslVerify=false", "push", "origin", "main"]
    res = destinations(tmp_git_repo, seg)
    assert res == ["https://github.com/org/repo.git"]
...
def test_effective_remote_fallback(tmp_git_repo: Path):
    subprocess.run(["git", "-C", str(tmp_git_repo), "remote", "add", "upstream", "https://github.com/org/repo2.git"], check=True)
