...
class TestPermittedHookMissingLogsDebug:
    ...
    def test_directory_named_like_hook_is_skipped_not_executed(
        self, mock_workspace: Path, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestPostRunHookRecordContract:
    ...
    def test_removing_the_call_site_would_leave_no_record(
        ...
        def _silent_run_hook(name: str, **kwargs: object) -> None:
