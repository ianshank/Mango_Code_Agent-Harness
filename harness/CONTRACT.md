...
- **INV-13:** A “verified” result MUST include policy, test, sandbox, source, and tool-version digests. **Four of five are recordable** on the broker evidence path (policy, source as a digest-of-digests of the loop-start enforcement baseline, tool-version as backend name+version, test over the resolved verification command and collected node ids). **Sandbox remains unattestable** until an isolation backend lands (spec steps 8–9) or C-AEI-6 records that no available primitive enforces both filesystem and network isolation. `ProcessBackend` contains — pinned cwd, bounded runtime, capped output, filtered environment — but does not isolate: it confines neither the filesystem nor the network, so there is no sandbox digest to record and no result produced today claims the fifth digest (DEC-010). Isolation is a later capability profile; the primitive cannot be exercised on this repository's CI runners.
