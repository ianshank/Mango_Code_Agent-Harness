...
- **INV-13:** A “verified” result MUST include policy, test, sandbox, source, and tool-version digests. **Four of five are recordable** on the default broker evidence path (policy, source as a digest-of-digests of the loop-start enforcement baseline, tool-version as backend name+version, test over the resolved verification command and collected node ids). **The fifth (sandbox) is recordable on `LandlockBackend`** when filesystem and network isolation were both applied (DEC-069). The broker default remains `ProcessBackend`, which contains — pinned cwd, bounded runtime, capped output, filtered environment — but does not isolate, so no result on that path claims the fifth digest (DEC-010). C-AEI-6 remains the close if a future runner loses Landlock; it is not the close of this measurement. MG-E1 (SWE-ReX / OpenSandbox) may record the sandbox digest only when `capabilities().filesystem_isolation == enforced` (R-AEI-10; see `docs/architecture/mg-e1-swe-rex-opensandbox.md` and DEC-069 / DEC-010).
...
The control-plane workflow example, `harness/control-plane/required-workflow.example.yml`, intentionally contains `PIN_FULL_COMMIT_SHA`; adopters must replace each with a reviewed full action SHA in the independently protected onboarding change, and `validate_adoption.py` fails closed while a stack's `ci.yml` still carries the placeholder. It is the only workflow file that does: this repository's own workflows under `.github/workflows/` and both per-stack `harness/*/.github/workflows/ci.yml` have every `uses:` pinned to a full commit SHA since DEC-045, and `test_documentation_claims.py` asserts that the placeholder appears in exactly the file this paragraph names. JVM wrapper, lockfile and verification metadata must also be generated and reviewed. These are explicit blockers rather than silently insecure defaults.
...
A root-level `.governance/` now exists and holds exactly one tracked file, `traceability.json` — the repository-scoped traceability config DEC-065 created — while the per-stack roots of trust stay under `harness/node/.governance/` and `harness/jvm/.governance/`, and root-level scripts (`verify_zero_skips.py`, `remotes.py`) go on consulting those by explicit path. `.governance/**` is therefore **no longer a declared-dormant pattern**: it woke on that file and was reclassified out of `test_protected_path_liveness.py`'s `DORMANT_PATTERNS`, which is what DEC-056 predicted in as many words — *"`.governance/**` ceases to be a declared-dormant protected_paths pattern once the root directory is live"*. Leaving it declared dormant while it matched would be the stale-waiver shape `test_dormant_patterns_are_still_dormant` exists to catch. What has **not** changed is the push posture: there is still no root `.governance/allowed-remotes.txt`, so `remotes.py` fails closed on the missing file and agent-initiated `git push` stays blocked. DEC-005 recorded that absence as the control; DEC-056 supersedes the mechanism — a real root allowlist plus an explicit PreToolUse / agent-policy denial of agent pushes, independent of allowlist presence — but that work has not landed, so the absence is still what holds today. A future stack wiring its own root of trust should follow the per-stack pattern unless that posture is revisited with its own `docs/decisions/` record.
...
Since DEC-012 there is a read-side counterpart at the same granularity: `harness/shared/read_policy.read_denial_reason`, consulted by the orchestrator's `read_file` handler. It does not consult `protected_paths` — an agent has to read the Makefile and the policies to do its work, and reading is not writing — but it denies credential-bearing filenames (`.env*`, `.netrc`, `*.pem`, ...) and any `.git` path segment, composing the same filename alternation `command_actions.classify` already uses to grade `cat <credential-file>` as `secret_access` for `run_command`. Without it, `read_file` would have been a second, ungoverned door onto exactly the credentials the command classifier already refuses: mapped to the plain `read` action, it would have returned a secret directly rather than denying it. Since DEC-042 `apply_patch` consults this read policy **first**, before the write policy: its `matched 0 times` / `matched 1 times` reply answers a question about a file's contents, which over a file the read policy refuses is a substring oracle — a credential recovered a character at a time without ever calling the door that is gated. It is still not a third *mechanism*: it reaches no path `write_file` cannot reach, it grades as the same `write` action, and both transports ask the policy decision point for that action before either policy is read. What changed is that it is now gated on both sides rather than on the write side alone.
...
Governed execution signs evidence at the broker. `ExecutionBroker` accepts a constructor-injected `signing_key`; when evidence is enabled and neither that argument nor `AGENT_EVIDENCE_KEY` supplies a key, `execute_command` returns `BLOCKED` with a reason naming `AGENT_EVIDENCE_KEY` **before spawn**. The resolved key is passed into `EvidenceBuilder`; the broker path does not read the environment at `export()`.

`EvidenceBuilder` (`harness/shared/governance/evidence_manifest.py`) still resolves a key at `export()` from the constructor argument, then `AGENT_EVIDENCE_KEY`, for control-plane callers such as `publish_policy_artifact.py`. Constructor injection takes precedence. A missing key raises `ValueError` (not `OSError`) at `export()` time on that path. The insecure hardcoded fallback key was removed in v2.1; fail-closed behavior is mandatory. See `.mango/skills/evidence-signing/SKILL.md` for the reusable skill.

## Python compatibility gate

`check_py_compat.py` enforces that all first-party Python uses only syntax available in the minimum CI matrix version (currently 3.10; DEC-064 / `docs/specs/python-floor-310.md` moved the floor off 3.9). The minimum is derived from the workflow matrix rather than restated as a literal. It detects: PEP 604 union syntax (`X | Y`) when the resolved floor is below 3.10, `datetime.UTC` (3.11+), and annotated assignments (`ast.AnnAssign`) that use union types without `from __future__ import annotations`. Run via `make check-compat`.
