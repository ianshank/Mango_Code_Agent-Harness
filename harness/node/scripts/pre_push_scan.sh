...
[ -f "$ROOT/scripts/remotes.py" ] || block "shared remote normalizer missing" "restore scripts/remotes.py"
[ -f "$ROOT/.governance/allowed-remotes.txt" ] || block "remote allowlist missing" "restore/configure .governance/allowed-remotes.txt"
python3 "$ROOT/scripts/remotes.py" --check-url "$REMOTE_URL" --allowlist "$ROOT/.governance/allowed-remotes.txt" || block "remote '$REMOTE_NAME' is not approved" "use an approved destination or independently approve a policy change"
