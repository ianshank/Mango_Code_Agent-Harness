...
elif "PIN_FULL_COMMIT_SHA" in ci.read_text():
    fail.append("third-party action SHAs are not pinned yet")
a = root / ".governance/allowed-remotes.txt"
if not a.is_file() or not [x for x in a.read_text().splitlines() if x.strip() and not x.lstrip().startswith("#")]:
...
if not rot.is_file():
    ...
    try:
        d = json.loads(rot.read_text())
        declared = d.get("policy_sha256", "")
        if not d.get("external_policy_ref") or not re.fullmatch(r"[0-9a-fA-F]{64}", declared):
...
if (root / "build.gradle.kts").exists():
    for f in ("gradlew", "gradle.lockfile", "gradle/verification-metadata.xml"):
        if not (root / f).exists():
            fail.append(f"{f} missing")
if fail:
    print("adoption: BLOCKED", file=__import__("sys").stderr)
    for x in fail:
        print("  - " + x, file=__import__("sys").stderr)
    raise SystemExit(1)
