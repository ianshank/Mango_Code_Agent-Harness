...
| **INV-13** | Verified Digests | Four of five digests are recordable on the default broker evidence path (policy, source, tool-version, test). **Sandbox is recordable on `LandlockBackend`** when both filesystem and network isolation were applied (DEC-069). The broker default remains `ProcessBackend` and does not claim the fifth digest. AC-12 host inventory is `capability_probe.py` (not a `BackendCapabilities` record). | `pytest harness/shared/tests/test_evidence_record.py harness/shared/tests/test_execution_backend.py harness/shared/tests/test_capability_probe.py harness/shared/tests/test_landlock_backend.py harness/shared/tests/test_sandbox_policy.py harness/shared/tests/test_escape_corpus.py harness/shared/tests/regression/test_capability_probe_vocabulary_regression.py` |
